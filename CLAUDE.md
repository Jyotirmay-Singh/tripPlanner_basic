...
- [ ] Step 28: Atlas M0 + Network Access. Provision an M0 cluster (AWS Singapore), allow `0.0.0.0/0` in Network Access, and create a DB user; set `MONGO_URL` to the SRV string (URL-encode the password). `DB_NAME` unchanged.
- [ ] Step 29: Go live. Connect the repo to Render, paste secrets, deploy; verify `/api/health` 200, data persists in Atlas, and existing endpoints behave identically. Point `EXPO_PUBLIC_BACKEND_URL` at the Render URL and set backend `APP_URL` to the web URL (Resend forgot-PIN reset links).
