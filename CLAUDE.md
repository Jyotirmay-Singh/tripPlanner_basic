...
- [ ] Step 51: Additive read-only `GET /trips/{id}/spend-summary` in new `routes/spend.py` (RBAC via `_trip_or_404`), registered in `server.py`; live API + RBAC tests (`TestSpendSummaryAPI`).
- [ ] Step 52: Pure `src/spend.ts::rankSpend` + types + `src/__tests__/spend.test.ts` (desc order, ties, family/individual, empty/zero/single, fraction scaling).
- [ ] Step 53: `SpendBarChart.tsx` (react-native-svg, theme-driven single-hue intensity ramp, family/individual marker, min bar width, compact header, empty state).
