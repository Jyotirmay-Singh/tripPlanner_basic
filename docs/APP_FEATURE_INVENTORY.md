...
This is a code-first inventory of the repository as it existed on 2026-09-08. Plans, roadmap checkboxes, specs, user guides, and Claude instructions were treated as supporting evidence only. A feature is described as current only when a corresponding code path was found.
...
- **Verification run:** 523 self-contained backend tests passed; application-source Python
  compilation, TypeScript `--noEmit`, and ESLint over `frontend/app` + `frontend/src` passed; the
  full frontend Jest run passed 554/554 tests. Two narrow read-only Frankfurter requests confirmed
  the live INR/LKR and INR/NPR v2 response shape/provider attribution; live app HTTP/MongoDB flows
  were not run.
...
- `backend/server.py` is the FastAPI assembly point. It registers auth, trips, members, expenses,
  exchange rates, balances, reports, metadata, receipts, spend, payments, and chat routers under
  `/api` ([backend/server.py](../backend/server.py)).
...
| Partial payments | Implemented and verified | Receiver, any linked person in a receiving family wallet, or trip admin records/edits/deletes a payment up to the current recommendation. New enabled LKR/NPR writes are whole rupees; legacy decimals remain readable and note-editable. Live recommendations are separated from chronological history and may reroute after recomputation | `/trip/[id]/settle-up`; payment CRUD APIs | [backend/routes/payments.py](../backend/routes/payments.py#L24), [frontend/app/trip/[id]/settle-up.tsx](../frontend/app/trip/[id]/settle-up.tsx#L57) | Backend write-policy/routes/concurrency and frontend payment/modal/permission tests passed | Live optimistic-concurrency guard and authorization were not exercised against MongoDB |
...
| XLSX report | Implemented and verified | Downloads five tabs; Transactions retains canonical totals and FX/Exact audit metadata, while Payments adds exact/rounded balances, adjustments, routing metadata, and current whole-unit recommendations when enabled | Reports tab; `GET /report.xlsx?token=...` | [frontend/app/(tabs)/reports.tsx](<../frontend/app/(tabs)/reports.tsx>), [backend/routes/reports.py](../backend/routes/reports.py) | Report builder/layout, conversion metadata, and whole-unit projection workbook tests passed | HTTP download was not run; JWT appears in the URL query string |
| PDF report | Implemented and locally verified | Full landscape report; Transactions shows canonical/original FX audit data and an enabled whole-unit projection section exposes exact/rounded balances, adjustments, and current route | Reports tab; `GET /report.pdf?token=...` | [frontend/src/api.ts](../frontend/src/api.ts), [backend/services/report_pdf.py](../backend/services/report_pdf.py) | Generated PDF text verified FX and rounded-settlement audit metadata | Live browser download was not run; query-string JWT has the same exposure caveat as XLSX |
