...
This is a code-first inventory of the repository as it existed on 2026-09-14. Plans, roadmap checkboxes, specs, user guides, and Claude instructions were treated as supporting evidence only. A feature is described as current only when a corresponding code path was found.
...
- **Verification run:** 906 self-contained backend tests passed after deselecting 316 tests that
  require the live HTTP client and one test that fetches real Google certificates. TypeScript
  `--noEmit`, direct ESLint, and the full frontend Jest run passed, with 94/94 suites and 797/797
  tests. Generated XLSX/PDF fixtures were also inspected for integer formats, complete values,
  allocations, migration adjustments, and page bounds. Live HTTP/MongoDB transaction flows could
  not run because no local MongoDB server or Docker daemon was available.
...
| Storage | Purpose |
|---|---|
| `users` | Account identity, hashed password, global role label, verification and credential flags |
...
Indexes, compatibility migrations, admin seeding, member-id backfills, family-email demotion, and an idempotent trip-activity repair run during application lifespan startup ([backend/server.py](../backend/server.py#L22)). Activity repair takes the latest recoverable trip creation, expense creation, payment creation, or effective paid-settlement time; pending settlements are excluded. Startup is therefore not read-only against the database.
...
| Session persistence, logout, and route guard | Implemented and verified | Token/session is restored from AsyncStorage; logout resets navigation; signed-out users are redirected, and unfinished Google users are restricted to mandatory password setup | Root layout, Profile, avatar header | [frontend/app/_layout.tsx](../frontend/app/_layout.tsx), [frontend/src/AuthContext.tsx](../frontend/src/AuthContext.tsx) | Logout/auth-navigation and password-auth Jest tests passed; TypeScript passed | No refresh-token flow; an invalid/expired token clears the local session |
...
| Roster-first join, existing-person claims, and admin approval | Implemented and locally verified | The code preview lists unlinked standalone individuals and family members before new-profile choices. Exact Gmail matches claim immediately; blank/different-email selections wait without trip access for owner/admin approval. Requests support cancellation, optional rejection notes, privacy-safe push updates, and a 24-hour same-target retry cooldown; approval preserves member IDs and financial history. A successful request immediately awaits Android push synchronization, and the minimal eligibility endpoint includes zero-trip pending requesters | `/join-trip`, trip Members tab; `POST /trips/join/preview`, `POST /trips/join`, `POST /trips/join-requests`, `GET/DELETE /trips/join-requests/{request_id}`, `GET /trips/{trip_id}/join-requests`, approve/reject endpoints; `GET /push/eligibility` | [frontend/app/join-trip.tsx](../frontend/app/join-trip.tsx), [frontend/src/JoinRequestsPanel.tsx](../frontend/src/JoinRequestsPanel.tsx), [backend/routes/join_requests.py](../backend/routes/join_requests.py), [backend/services/join_requests.py](../backend/services/join_requests.py), [backend/routes/trips.py](../backend/routes/trips.py), [backend/routes/push.py](../backend/routes/push.py) | Request lifecycle/race/history tests, join screen/panel tests, identity helpers, push eligibility/registration tests, notification routing/recipient tests, TypeScript, and backend compile checks pass; live API/MongoDB tests were not run | A user can have one active request per trip; rejected same-target retries wait 24 hours; history-bearing identities cannot be detached; legacy action-less join API behavior remains |
