...
| Trip create/list/view/edit/delete and share code | Implemented but unverified | Users create trips with date range, budget, immutable official currency, and individual/family self-identity; share a six-character code; admins edit non-currency settings; owner deletes | `/create-trip`, Trips/Home, `/trip/[id]`; trip CRUD APIs | [frontend/app/create-trip.tsx](../frontend/app/create-trip.tsx), [backend/routes/trips.py](../backend/routes/trips.py) | Date/identity/currency-lock helper and route tests passed; live CRUD was not run | Different reporting currencies require separate trips |
