...
| Multi-currency expense conversion | Implemented and locally verified; rollout disabled by default | A transaction may be entered in any supported currency and converted to the immutable trip currency with the expense-date Frankfurter v2 rate or an explicit manual/card rate/final amount. Weekend fallback shows the effective date; approval locks amount/rate/source/version; unrelated edits do not revalue history | Add/Edit Transaction conversion panel; `GET /exchange-rates/quote`; expense create/edit/reconvert APIs | [backend/services/exchange_rates.py](../backend/services/exchange_rates.py), [backend/services/expense_conversion.py](../backend/services/expense_conversion.py), [frontend/src/ui/ExchangeRatePanel.tsx](../frontend/src/ui/ExchangeRatePanel.tsx) | Mocked provider/cache/error/service/route tests, quote hook/panel/payload tests, report tests, TypeScript, and compile checks passed | `MULTI_CURRENCY_EXPENSES_ENABLED` defaults false; live provider/Mongo concurrency was not exercised; canonical ledger remains fixed at two decimals |
...
| XLSX report | Implemented and verified | Downloads five tabs; Transactions retains canonical totals and adds original amount/currency, locked rate/effective date/provider/mode, and original Exact allocations | Reports tab; `GET /report.xlsx?token=...` | [frontend/app/(tabs)/reports.tsx](<../frontend/app/(tabs)/reports.tsx>), [backend/routes/reports.py](../backend/routes/reports.py) | Report builder/layout and conversion metadata tests passed | HTTP download was not run; JWT appears in the URL query string; reports cap fetched rows at 5,000 |
| PDF report | Implemented and locally verified | Full landscape report; Transactions shows canonical/original values and compact FX audit metadata while totals remain canonical | Reports tab; `GET /report.pdf?token=...` | [frontend/src/api.ts](../frontend/src/api.ts), [backend/services/report_pdf.py](../backend/services/report_pdf.py) | Generated PDF was rendered in tests and extracted text verified original/rate/effective-date/provider metadata | Live browser download was not run; query-string JWT has the same exposure caveat as XLSX |
...
| Health/config/categories metadata | Implemented and verified | Public health check, category list, and runtime email/chat/multi-currency capability flags | `GET /health`, `/meta/categories`, `/meta/config` | [backend/routes/meta.py](../backend/routes/meta.py) | Direct route and AuthContext flag tests passed; live HTTP not run | Health deliberately does not prove database or provider health |
...
- **Canonical precision is fixed at two decimals.** LKR and NPR are correct under this rule, but the
  picker also includes ISO currencies with zero or three minor units. Currency-specific minor units
