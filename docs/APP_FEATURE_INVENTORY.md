...
| Android encrypted local store foundation | Implemented locally; native runtime unverified | Account-scoped SQLCipher schema and SecureStore key support cached identity, complete confirmed read snapshots, and a future durable outbox. Missing keys and failed migrations lock access without clearing queued rows; account deletion purges that account's rows | Android native adapter | [frontend/src/offlineStore.android.ts](../frontend/src/offlineStore.android.ts), [frontend/src/offlineSchema.ts](../frontend/src/offlineSchema.ts), [frontend/app.base.json](../frontend/app.base.json) | Store/migration/key-loss/account-isolation and atomic read-snapshot Jest tests, TypeScript, ESLint, and online web export passed | A native development build and device test are still needed to prove SQLCipher; offline money writes are not active |
| Android saved trip and confirmed financial views | Implemented locally; native runtime unverified | Previously opened trips, roster, expenses, server balances, spend, and manual-payment history reopen from a complete encrypted snapshot with a last-sync label. Partial refreshes retain the old complete view; missing copies are unavailable rather than zero. The dashboard and trip picker retain account-scoped list/overview copies. Expense and payment actions remain online-only; UPI attempts are not cached | Dashboard, Trips, Add picker, trip detail, Add Expense, Settle Up | [frontend/src/offlineReads.ts](../frontend/src/offlineReads.ts), [frontend/src/OfflineReadStatus.tsx](../frontend/src/OfflineReadStatus.tsx), [frontend/app/trip/[id]/settle-up.tsx](<../frontend/app/trip/[id]/settle-up.tsx>) | Focused cold-load, partial-refresh, stale-view, privacy, account-scope, UPI-unavailable, and web-only tests; full Jest, TypeScript, lint, and 40-route web export passed | Native airplane-mode/force-stop test is pending; no queued expense or payment capture is shipped |
