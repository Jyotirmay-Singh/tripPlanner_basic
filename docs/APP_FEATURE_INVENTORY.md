...
This is a code-first inventory of the repository as it existed on 2026-09-12. Plans, roadmap checkboxes, specs, user guides, and Claude instructions were treated as supporting evidence only. A feature is described as current only when a corresponding code path was found.
...
- **Git branch:** `feature/upi-recipient-confirmation-step-5`.
- **Commit before this feature work:** `f33c760e6824bcbcfa8a97894d6f2d1bb6153ded`.
- **Repository state at feature start:** the pre-existing local edits to
  `upi_payment_implementation_plan.md` and `.claude/settings.local.json` were deliberately preserved
  outside this feature's changes.
- **Verification run:** 791 self-contained backend tests passed; application-source Python
  compilation, TypeScript `--noEmit`, and ESLint over `frontend/app` + `frontend/src` passed; the
  full frontend Jest run passed 761/761 tests. The focused Step 5 runs passed 21 backend
  payment-attempt tests and 62 frontend handoff/activity/API/routing tests. Live HTTP/MongoDB
  transaction flows could not run because no local MongoDB server or Docker daemon was available.
...
| Exact per-person split | Implemented and verified | The author assigns positive allocation magnitudes in the transaction's original currency; refunds preserve a negative canonical sign. The backend validates the original total, converts with the locked rate, and uses deterministic largest-remainder rounding so canonical shares foot exactly | Add/Edit Exact editor; expense APIs | [backend/services/custom_split.py](../backend/services/custom_split.py), [backend/routes/expenses.py](../backend/routes/expenses.py), [frontend/src/ExactSplitEditor.tsx](../frontend/src/ExactSplitEditor.tsx) | Backend exact/conversion/share/report tests and frontend exact/payload tests passed | Live create/edit MongoDB round-trip was not exercised |
...
| Precise balance ledger, conserving settlement projection, and family breakdown | Implemented and locally verified; whole-unit rollout disabled by default | Stored trip-currency expenses and effective payments build a zero-sum 12-decimal scaled vector. LKR/NPR can jointly project it to whole rupees with disclosed per-member adjustments; bounded small groups get a proven minimum-transfer route and larger/search-limited groups get a deterministic bounded greedy route | Balances and Settle Up; `GET /trips/{id}/balances` | [backend/utils/balances.py](../backend/utils/balances.py#L1), [backend/services/settlement_engine.py](../backend/services/settlement_engine.py), [frontend/src/settlementProjection.ts](../frontend/src/settlementProjection.ts) | Unit/property-style/adversarial projection, lifecycle, report, frontend, TypeScript, and performance coverage passed | `WHOLE_UNIT_SETTLEMENTS_ENABLED` defaults false; exact route limit is 12 non-zero entities/100,000 states; live Mongo aggregation was not exercised |
| Legacy settlement lifecycle | Deprecated or disabled | APIs can still create completed settlements or pending records and mark pending records paid; new enabled LKR/NPR amounts must be whole rupees while legacy decimals can still be marked paid | `POST /settle`; settlement list/create/patch APIs | [backend/models/settlement.py](../backend/models/settlement.py#L6), [backend/routes/balances.py](../backend/routes/balances.py#L24) | Focused settlement policy/route tests passed; live APIs not run | Current settle-up UI uses `payments`, not these endpoints; retained for compatibility |
| Partial payments | Implemented and verified | Receiver, any linked person in a receiving family wallet, or trip admin records/edits/deletes a payment up to the current recommendation. New enabled LKR/NPR writes are whole rupees; legacy decimals remain readable and note-editable. Live recommendations are separated from chronological history and may reroute after recomputation. Manual balance-changing payment/paid-settlement writes share the trip revision when Mongo transactions are available | `/trip/[id]/settle-up`; payment CRUD APIs | [backend/routes/payments.py](../backend/routes/payments.py), [backend/routes/balances.py](../backend/routes/balances.py), [frontend/app/trip/[id]/settle-up.tsx](../frontend/app/trip/[id]/settle-up.tsx) | Backend write-policy/routes/concurrency and frontend payment/modal/permission tests passed | Standalone MongoDB retains the historical non-transaction fallback for manual writes |
...
| XLSX report | Implemented and verified | Downloads five tabs; Transactions retains canonical totals and FX/Exact audit metadata, while Payments includes a sanitized source label for recipient-confirmed UPI rows plus exact/rounded balances, adjustments, routing metadata, and current whole-unit recommendations when enabled | Reports tab; `GET /report.xlsx?token=...` | [frontend/app/(tabs)/reports.tsx](<../frontend/app/(tabs)/reports.tsx>), [backend/routes/reports.py](../backend/routes/reports.py) | Report builder/layout, conversion metadata, source labeling, and whole-unit projection workbook tests passed | HTTP download was not run; JWT appears in the URL query string; UPI IDs/references are intentionally omitted |
| PDF report | Implemented and locally verified | Full landscape report; Transactions shows canonical/original FX audit data, Payments labels recipient-confirmed UPI rows without private UPI/reference data, and an enabled whole-unit projection section exposes exact/rounded balances, adjustments, and current route | Reports tab; `GET /report.pdf?token=...` | [frontend/src/api.ts](../frontend/src/api.ts), [backend/services/report_pdf.py](../backend/services/report_pdf.py) | Generated PDF text verified FX, sanitized UPI source, and rounded-settlement audit metadata | Live browser download was not run; query-string JWT has the same exposure caveat as XLSX |
...
| Health/config/categories metadata | Implemented and verified | Public health check, category list, and runtime email/chat/multi-currency/whole-unit capability flags | `GET /health`, `/meta/categories`, `/meta/config` | [backend/routes/meta.py](../backend/routes/meta.py) | Direct route and AuthContext flag tests passed; live HTTP not run | Health deliberately does not prove database or provider health |
...
- **Stored amount precision and settlement precision differ deliberately.** Canonical converted expense
  amounts remain locked at their write-time precision; settlement parses them into a 12-decimal scaled
  ledger so split fractions conserve exactly. The compatibility display remains two-decimal, while
  enabled LKR/NPR recommendations use a separate one-rupee projection. Other currency-specific
  settlement increments are deliberately deferred.
