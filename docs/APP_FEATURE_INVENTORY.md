...
| Per-trip realtime text chat | Implemented and verified locally | A fifth trip tab provides MongoDB-backed text history, trip-person sender labels, exact cross-device unread counts, optimistic retry, sender edit/delete tombstones, reconnect catch-up, owner-only clearing, and privacy-safe Android push for new messages | Trip Chat tab; chat REST APIs and authenticated WebSocket | [frontend/src/TripChat.tsx](../frontend/src/TripChat.tsx#L1), [frontend/src/useTripChat.ts](../frontend/src/useTripChat.ts#L1), [backend/routes/chat.py](../backend/routes/chat.py#L1), [backend/services/push_notifications.py](../backend/services/push_notifications.py) | Backend chat tests plus frontend helper, component, tab, push payload/routing, TypeScript, lint, and full Jest verification passed | Live HTTP/MongoDB/WebSocket/push round-trip remains device-QA gated; realtime fan-out is intentionally process-local while production uses one worker; text-only with no presence/media |
...
| Expense/refund CRUD, categories, date/time, and ordering | Implemented and locally verified | Any trip member can add a signed whole-unit transaction; creator/admin can edit/delete; negative nonzero amounts represent money returned; old-client decimals are normalized half-up and audited; lists sort newest first by parsed date/time | Add/Edit Transaction and Expenses tab; expense CRUD APIs | [backend/models/expense.py](../backend/models/expense.py), [backend/routes/expenses.py](../backend/routes/expenses.py), [frontend/src/expenseSort.ts](../frontend/src/expenseSort.ts) | Conversion-aware route, ordering, payload, whole-unit policy, TypeScript, and focused UI tests passed | Live MongoDB CRUD was not run |
