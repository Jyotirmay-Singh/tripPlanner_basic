...
|---|---|---|---|---|---|---|
| Revocable Android invite links | Implemented; live behind kill switch | Every trip member can rotate one private seven-day link. Installed Android apps open the token-aware join preview; Android browsers stay on a manual Open App / Android APK download / Continue on web landing page; authenticated desktop/iOS web users enter the preview automatically. Existing members go to trip Summary and non-members see the Join Trip flow | `/invite/[token]`, trip Members tab; invite management/status APIs; token-aware join APIs | [backend/services/invites.py](../backend/services/invites.py), [frontend/app/invite/[token].tsx](../frontend/app/invite/[token].tsx), [frontend/src/InviteLinksPanel.tsx](../frontend/src/InviteLinksPanel.tsx) | Invite security/API, member/admin authorization, rotation/concurrency, manual-only landing, runtime-refresh, exact share copy, navigation, auth-return, member routing, and join tests plus TypeScript | Build 8 gains secure links from `INVITE_LINKS_ENABLED=true`, but its native share text remains the older invite-only wording; the full link + code + Android-download message is live on web and reaches Android in the next normal APK release |
