...
|---|---|---|---|---|---|---|
| Revocable Android invite links | Implemented locally; rollout gated | Owners/admins create seven-day links; installed Android apps open the token-aware join wizard, while browsers offer the stable APK and web flow. Links survive authentication and can be audited/revoked | `/invite/[token]`, trip Members tab; invite management/status APIs; token-aware join APIs | [backend/services/invites.py](../backend/services/invites.py), [frontend/app/invite/[token].tsx](../frontend/app/invite/[token].tsx), [frontend/src/InviteLinksPanel.tsx](../frontend/src/InviteLinksPanel.tsx) | Invite security/API, landing, navigation, auth-return, join, and management tests plus TypeScript | `INVITE_LINKS_ENABLED` stays false until a verified App-Link-capable APK is at `/download/android`; legacy member code sharing remains available |
