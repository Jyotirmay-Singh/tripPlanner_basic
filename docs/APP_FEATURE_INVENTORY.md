...
- **Git branch:** `main`.
- **Commit before this feature work:** `76f73d95f3d03f3cacac6d4a9a459f6d4ba7a20d`.
- **Repository state at feature start:** pre-existing local edits, deleted assets, and untracked user
  files were deliberately preserved outside the whole-unit feature's changes.
- **Verification run:** 907 self-contained backend tests passed; test files requiring the live HTTP
  service/MongoDB and the test that fetches real Google certificates were excluded. Python source
  compilation, TypeScript `--noEmit`, direct ESLint, and the Expo web export passed. The full
  frontend Jest run passed with 98/98 suites and 830/830 tests. Generated XLSX/PDF fixtures were
  also inspected for integer formats, complete values, allocations, migration adjustments, and
  page bounds. Live HTTP/MongoDB transaction flows could not run because no local MongoDB server or
  Docker daemon was available.

## Application Summary

Trip Splitter is a collaborative trip-expense application for mobile and web. It serves groups whose costs may be attributed to standalone people or family units. Authenticated users create or join trips, chat under their trip-specific person identity, record positive expenses and negative refunds in the trip currency or a converted foreign currency, split each transaction per person, per family, or by exact per-person amounts, inspect balances, record partial payments, attach receipt images, and export reports.
...
- `backend/server.py` is the FastAPI assembly point. It registers auth, trips, members, expenses,
  exchange rates, balances, reports, metadata, receipts, spend, payments, payment attempts, and chat
  routers under `/api` ([backend/server.py](../backend/server.py)).
...
| Storage | Purpose |
|---|---|
| `users` | Account identity, hashed password, global role label, verification and credential flags, optional canonical E.164 mobile profile, and optional UPI ID |
...
Indexes, compatibility migrations, admin seeding, member-id backfills, family-email demotion, mobile-profile null backfills, mobile-claim reconciliation, and an idempotent trip-activity repair run during application lifespan startup ([backend/server.py](../backend/server.py#L22)). Activity repair takes the latest recoverable trip creation, expense creation, payment creation, or effective paid-settlement time; pending settlements are excluded. Mobile reconciliation repairs standalone-database interruptions and removes stale claims after roster changes. Startup is therefore not read-only against the database.
...
- Registration and all identity-email inputs are Gmail-only; local passwords are at least nine characters ([backend/routes/auth.py](../backend/routes/auth.py#L61)).
...
- Optional mobile profiles are validated internationally, normalized to E.164, and limited to mobile or ambiguous fixed-line/mobile classifications. A number may be reused globally, but never by two linked people in the same trip.
...
- **Frankfurter v2:** keyless historical reference-rate provider, contacted only by the FastAPI
  backend. Deterministic tests mock it; no automatic provider failover is configured.
...
No external deployment, OAuth exchange, email delivery, hosted database, or live Frankfurter request
was contacted during this audit.
...
|---|---|---|---|---|---|---|
| Stable Android invite link | Implemented; live behind kill switch | Every trip has one private link that all members can copy or share; owners/admins can reset it immediately. Installed Android apps open the token-aware join preview; Android browsers stay on a manual Open App / Android APK download / Continue on web landing page; authenticated desktop/iOS web users enter the preview automatically. Existing members go to trip Summary and non-members see the Join Trip flow | `/invite/[token]`, trip Members tab; singleton link/reset APIs; token-aware join APIs | [backend/services/invites.py](../backend/services/invites.py), [frontend/app/invite/[token].tsx](../frontend/app/invite/[token].tsx), [frontend/src/InviteLinksPanel.tsx](../frontend/src/InviteLinksPanel.tsx) | Signed-token security, stable-link/reset behavior, member/admin authorization, copy/share actions, navigation, auth-return, member routing, and join tests plus TypeScript | The runtime flag can disable link retrieval/resolution without affecting the permanent six-character trip code or existing members |
| Gmail registration, password login, JWT session | Implemented and locally verified | Users register with name, Gmail, and password; subsequent login uses the remembered email plus password | `(auth)/register`, `(auth)/login`; `POST /auth/register`, `POST /auth/login`, `GET /auth/me` | [backend/routes/auth.py](../backend/routes/auth.py), [frontend/src/AuthContext.tsx](../frontend/src/AuthContext.tsx) | Focused backend and frontend authentication tests plus TypeScript pass; live API tests were not run | Requires MongoDB and correctly configured JWT secret; Gmail-only is a product restriction |
| Optional international mobile onboarding and trip contacts | Implemented and locally verified | Registration and explicit login offer mobile setup when no number is saved; “Not now” skips only the current session, while restored sessions stay uninterrupted. Profile supports add/change/cancel/confirmed removal. Authorized trip detail shows current numbers only for linked people; tapping one offers Call and Copy | `/set-mobile`, Profile, trip Members tab; `PATCH /auth/me/mobile`, authenticated `GET /trips/{id}` | [backend/utils/mobile_numbers.py](../backend/utils/mobile_numbers.py), [backend/services/mobile_claims.py](../backend/services/mobile_claims.py), [frontend/app/set-mobile.tsx](../frontend/app/set-mobile.tsx), [frontend/src/MobileNumberInput.tsx](../frontend/src/MobileNumberInput.tsx) | International normalization/type checks, conflict/rollback/enrichment tests, auth/navigation/Profile/member-contact Jest tests, TypeScript, and frontend lint pass | No OTP is sent and no verification badge is shown. Public invites, trip lists, audits, PDF/XLSX reports, and other exports never include mobile numbers; live Mongo concurrency still needs integration verification |
| Optional UPI onboarding and Profile details | Implemented and locally verified | New password accounts can save or skip a UPI ID; first-time Google accounts see the same optional step after password creation. Existing sign-ins/restored sessions are not prompted. Profile displays the saved ID or “UPI ID not set” and supports add, edit, cancel, and confirmed removal | `/set-upi`, Profile; `PATCH /auth/me` | [frontend/app/set-upi.tsx](../frontend/app/set-upi.tsx), [frontend/app/(tabs)/profile.tsx](<../frontend/app/(tabs)/profile.tsx>), [frontend/src/AuthContext.tsx](../frontend/src/AuthContext.tsx), [backend/routes/auth.py](../backend/routes/auth.py) | Focused onboarding/auth/invite/Profile Jest tests, TypeScript, frontend source lint, and backend UPI profile tests pass | UPI syntax is not proof of ownership; ownership and bank settlement are never inferred from syntax |
| Google sign-in and first-time password setup | Implemented but externally unverified | Android uses Credential Manager with the Web OAuth audience; web/iOS retain AuthSession; new OAuth users must create a local password before entering protected screens | `GoogleSignInButton.android`, `GoogleSignInButton`, `/set-credentials`; `POST /auth/google`, `POST /auth/set-credentials` | [frontend/src/GoogleSignInButton.android.tsx](../frontend/src/GoogleSignInButton.android.tsx), [frontend/src/GoogleSignInButton.tsx](../frontend/src/GoogleSignInButton.tsx), [backend/routes/auth.py](../backend/routes/auth.py) | Focused Google routing/setup tests and TypeScript pass; external OAuth still requires package/SHA and production audience verification | Hidden in Expo Go or when the required client ID is absent; backend must accept the Web audience used by Android |
| Email verification and password reset | Implemented and locally verified | Verification and single-use password-reset links are available; “Forgot password?” appears only when backend email delivery is enabled | `/verify-email`, `(auth)/forgot-password`, `/reset-password`; verification/reset APIs | [backend/routes/auth.py](../backend/routes/auth.py), [frontend/src/UnverifiedBanner.tsx](../frontend/src/UnverifiedBanner.tsx#L1) | Focused frontend and backend email-flow tests pass | The entire email feature can be runtime-disabled |
| Signed-in password change | Implemented but live-unverified | Profile links to the existing password-change screen | `/change-password`; `POST /auth/change-password` | [backend/routes/auth.py](../backend/routes/auth.py) | TypeScript and related backend tests pass; live account mutation was not exercised | Requires the current password |
| Session persistence, logout, and route guard | Implemented and locally verified | Android migrates its existing token to SecureStore and can restore a previously verified identity during a transient outage until token expiry; web and iOS retain the existing online session path. Logout resets navigation and warns when pending Android actions exist. Setup precedence remains required Google password, optional mobile, optional UPI, then invitation/dashboard | Root layout, Profile, Login | [frontend/app/_layout.tsx](../frontend/app/_layout.tsx), [frontend/src/AuthContext.tsx](../frontend/src/AuthContext.tsx), [frontend/src/tokenStorage.android.ts](../frontend/src/tokenStorage.android.ts) | Auth, token migration, restart, 401, expiry, logout, onboarding, and invite-handoff Jest tests plus TypeScript and web export passed | Native Android process-restart verification remains pending; there is no refresh-token flow |
| Android encrypted local store foundation | Implemented locally; native runtime unverified | Account-scoped SQLCipher/SecureStore store with transactional schema v5. Confirmed copies and reconciled outbox rows expire after 30 days; unresolved actions and their trip copies are retained. Missing keys and failed migrations fail closed. Android Auto Backup is disabled | Android native adapter | [frontend/src/offlineStore.android.ts](../frontend/src/offlineStore.android.ts), [frontend/src/offlineSchema.ts](../frontend/src/offlineSchema.ts), [frontend/src/offlineRetention.ts](../frontend/src/offlineRetention.ts) | Session 8 Jest includes real SQLite retention/migration checks on Node 24; TypeScript, lint, and resolved Expo config passed | Native SQLCipher, merged manifest, key restoration, and storage-full behavior remain device gates; offline writes are off |
| Android saved trip and confirmed financial views | Implemented locally; native runtime unverified | Previously opened trips, roster, expenses, server balances, spend, and manual-payment history reopen from a complete saved snapshot with a last-sync label. Partial refreshes retain the previous complete view; missing copies are unavailable rather than zero. Pending actions do not alter confirmed totals | Dashboard, Trips, Add picker, trip detail, Add Expense, Settle Up | [frontend/src/offlineReads.ts](../frontend/src/offlineReads.ts), [frontend/src/OfflineReadStatus.tsx](../frontend/src/OfflineReadStatus.tsx) | Session 8 full Jest passed 115 suites/956 tests; TypeScript, lint, and 42-route online web export passed | Force-stop/airplane-mode native test is pending; web and iOS remain online-only |
| Android offline expense and manual-payment outbox | Implemented behind ordinary-build default-off flag; QA APK available, release-unverified | Same-currency expenses/refunds and manual records use stable mutation UUIDs, pending/review states, ordered foreground retry, and server-confirmed reconciliation. Unresolved items never enter confirmed totals or retention cleanup; UPI remains online | Pending expense/payment screens and Trips queue | [frontend/src/syncWorker.ts](../frontend/src/syncWorker.ts), [frontend/src/offlineActivation.ts](../frontend/src/offlineActivation.ts), [OFFLINE_ANDROID_QA_RUNBOOK.md](OFFLINE_ANDROID_QA_RUNBOOK.md) | Session 8 backend selection passed 321 tests with 0 skips against an isolated MongoDB replica set; both live transaction/replay tests passed. Frontend 115 suites/956 tests passed. On 2026-09-26, a local QA APK passed package, signature, bundled loopback URL, native library, and merged-manifest checks; an isolated backend returned health `ok` and both protocol versions `1` | Native journey, live API authorization, second-device view, upgrade migration, and failure matrix remain open; keep ordinary/release capture off |
| Account deletion and privacy cleanup | Implemented and locally verified | Profile opens a dedicated impact review. Trips default to Keep; eligible cards can Leave or Dissolve family. Unsettled retained positions require acknowledgement, uppercase `DELETE`, and a final confirmation. Successful deletion clears local auth/invite/email state; a later registration with the same Gmail is a new account with no restored trip access | `/delete-account`; `GET /auth/me/deletion-impact`, `DELETE /auth/me` | [backend/services/departure.py](../backend/services/departure.py), [backend/routes/departure.py](../backend/routes/departure.py), [frontend/app/delete-account.tsx](../frontend/app/delete-account.tsx), [frontend/src/AuthContext.tsx](../frontend/src/AuthContext.tsx) | Focused backend identity/settlement/ownership/privacy/transaction tests and frontend review/session-reset tests pass; TypeScript passes | Destructive writes require MongoDB transactions and return 503 when unavailable. Live replica-set concurrency and complete production-data cleanup remain integration-QA gates; the fixed super-admin is non-deletable |
| Trip create/list/view/edit/delete and share code | Implemented and locally verified | Users create trips with date range, budget, immutable official currency, and individual/family self-identity; share a six-character code; admins edit non-currency settings; owner deletes. Trip lists are scoped to the signed-in user and deterministically ordered by qualifying participant activity, then creation time and ID | `/create-trip`, Trips/Home, `/trip/[id]`; trip CRUD APIs | [frontend/app/create-trip.tsx](../frontend/app/create-trip.tsx), [backend/routes/trips.py](../backend/routes/trips.py), [backend/services/trip_activity.py](../backend/services/trip_activity.py) | Activity initialization, monotonic touch, financial backfill, deterministic scoped ordering, and existing date/identity/currency-lock route tests pass; live CRUD was not run | Different reporting currencies require separate trips; historical roster/descriptive activity predating `last_activity_at` cannot be reconstructed |
| Roster-first join, existing-person claims, and admin approval | Implemented and locally verified | The code preview lists unlinked standalone individuals and family members before new-profile choices. Exact Gmail matches claim immediately; blank/different-email selections wait without trip access for owner/admin approval. Every direct/approved link reserves the account's mobile claim first, so a duplicate number in that trip returns a named conflict without changing membership. Approval preserves member IDs and financial history | `/join-trip`, trip Members tab; join, claim, and approve APIs | [frontend/app/join-trip.tsx](../frontend/app/join-trip.tsx), [frontend/src/JoinRequestsPanel.tsx](../frontend/src/JoinRequestsPanel.tsx), [backend/services/join_requests.py](../backend/services/join_requests.py), [backend/routes/trips.py](../backend/routes/trips.py), [backend/services/mobile_claims.py](../backend/services/mobile_claims.py) | Request lifecycle/race/history tests, explicit mobile-conflict recovery, join screen/panel tests, and focused claim tests pass; live API/MongoDB tests were not run | A user can have one active request per trip; rejected same-target retries wait 24 hours; history-bearing identities cannot be detached; legacy action-less join API behavior remains |
| Member/family administration and contact identity | Implemented and locally verified | Admins add/edit/remove people and families; family sub-members have stable IDs, optional Gmail contacts, server-managed linked accounts, and linked-user mobile contact lines beneath each person. Removing a linked person releases their claim | Trip Members tab and add/edit/manage-member screens; member APIs | [backend/models/member.py](../backend/models/member.py#L6), [backend/routes/members.py](../backend/routes/members.py#L41), [frontend/app/trip/[id]/index.tsx](<../frontend/app/trip/[id]/index.tsx>) | Existing member/email coverage plus mobile enrichment, aligned family rendering, Call/Copy, failure-feedback, and claim-release tests pass | A family entity itself cannot own an email/account; manual/unclaimed people have no mobile contact; removal is blocked while balances remain |
| Self-service trip departure and family dissolution | Implemented and locally verified | The Members tab shows the linked identity, exact settlement state, ownership outcome, and Leave action. Standalone departure requires exact zero; family departure requires an exact-zero family entity and every family row settled. A sole linked family user can keep non-app people or explicitly dissolve the family; a sole person must dissolve. History is frozen and retained while trip-scoped account/UPI references and chat identity are scrubbed | Trip Members tab; `GET /trips/{id}/membership/leave-impact`, `DELETE /trips/{id}/membership` | [backend/services/departure.py](../backend/services/departure.py), [frontend/src/MembershipCard.tsx](../frontend/src/MembershipCard.tsx), [backend/services/settlement_engine.py](../backend/services/settlement_engine.py) | Focused exact-position, family-choice, ownership, UPI blocker, historical EXACT split, component, and navigation tests pass | Active UPI attempts block departure. Family dissolution is unavailable while another linked app user remains. Required MongoDB transactions fail closed when unavailable |
| Per-trip realtime text chat | Implemented and verified locally | A fifth trip tab provides MongoDB-backed text history, trip-person sender labels, exact cross-device unread counts, optimistic retry, sender edit/delete tombstones, reconnect catch-up, owner-only clearing, and Android push that shows the snapshotted sender and trip but never chat text | Trip Chat tab; chat REST APIs and authenticated WebSocket | [frontend/src/TripChat.tsx](../frontend/src/TripChat.tsx#L1), [frontend/src/useTripChat.ts](../frontend/src/useTripChat.ts#L1), [backend/routes/chat.py](../backend/routes/chat.py#L1), [backend/services/push_notifications.py](../backend/services/push_notifications.py) | Backend chat tests plus frontend helper, component, tab, push payload/routing, TypeScript, lint, and full Jest verification passed | Lock-screen copy may expose the sender and trip name; live HTTP/MongoDB/WebSocket/push round-trip remains device-QA gated; realtime fan-out is intentionally process-local while production uses one worker; text-only with no presence/media |
...
| Expense/refund CRUD, categories, date/time, and ordering | Implemented and locally verified | Any trip member can add a signed whole-unit transaction; creator/admin can edit/delete; negative nonzero amounts represent money returned; old-client decimals are normalized half-up and audited; lists sort newest first by parsed date/time. Android activity push shows the creator plus quoted description/category and trip, but no amount, split, or receipt | Add/Edit Transaction and Expenses tab; expense CRUD APIs | [backend/models/expense.py](../backend/models/expense.py), [backend/routes/expenses.py](../backend/routes/expenses.py), [frontend/src/expenseSort.ts](../frontend/src/expenseSort.ts) | Conversion-aware route, ordering, payload, whole-unit policy, rich-notification, TypeScript, and focused UI tests passed | Lock-screen copy may expose the creator, expense description/category, and trip name; live MongoDB CRUD was not run |
| Multi-currency expense conversion | Implemented and locally verified; rollout disabled by default | A transaction may be entered in any supported currency and converted to the immutable trip currency with the expense-date Frankfurter v2 rate or an explicit manual/card rate/final amount. Source and converted money are whole units; exchange-rate ratios retain decimal precision. Weekend fallback shows the effective date; approval locks amount/rate/source/version; unrelated edits and settlement recomputation do not revalue immutable history | Add/Edit Transaction conversion panel; `GET /exchange-rates/quote`; expense create/edit/reconvert APIs | [backend/services/exchange_rates.py](../backend/services/exchange_rates.py), [backend/services/expense_conversion.py](../backend/services/expense_conversion.py), [frontend/src/ui/ExchangeRatePanel.tsx](../frontend/src/ui/ExchangeRatePanel.tsx) | Mocked provider/cache/error/service/route tests, quote hook/panel/payload tests, report tests, TypeScript, and compile checks passed | `MULTI_CURRENCY_EXPENSES_ENABLED` defaults false; live provider/Mongo concurrency was not exercised; stored canonical whole-unit amounts remain the sole settlement input |
| Budget warning and force-save | Implemented and locally verified | Creating or amount-reconverting an expense over budget returns a confirmation response and can be force-saved; refunds reduce net spend; only canonical trip-currency amounts enter the check | Add/Edit Transaction; expense create/edit/reconvert APIs | [backend/routes/expenses.py](../backend/routes/expenses.py) | Conversion route and budget-oriented legacy coverage passed | Live MongoDB aggregation was not exercised |
| Per-capita split | Implemented and verified | A whole-unit cost is divided proportionally among participating humans, including selected family sub-members and weight snapshots. Remainders go to the eligible payer first, then visible roster order | Add/Edit split selector; balances and reports | [backend/services/calculator.py](../backend/services/calculator.py#L119), [backend/services/expense_shares.py](../backend/services/expense_shares.py#L78) | Payer-included/excluded, family participation, refunds, expense-share, report, and frontend preview tests passed | Family size/participation semantics are complex; live API persistence was not exercised |
| Per-family split | Implemented and verified | Each selected root entity receives a whole-unit proportional allocation regardless of family size, with payer-first then visible-roster remainder handling | Add/Edit split selector; balances and reports | [backend/services/calculator.py](../backend/services/calculator.py#L134) | Per-family, remainder, and report tests passed ([test_per_family.py](../backend/tests/test_per_family.py#L6)) | Intra-family display breakdown is separate from the entity ledger; family sub-allocations use participating roster order |
| Exact per-person split | Implemented and verified | Updated clients assign whole allocation magnitudes that sum exactly to the whole total. The server accepts legacy decimal submissions, jointly rounds them, and reconciles remainder units payer-first then in visible roster order without negative allocations; refunds preserve the canonical sign | Add/Edit Exact editor; expense APIs | [backend/services/custom_split.py](../backend/services/custom_split.py), [backend/routes/expenses.py](../backend/routes/expenses.py), [frontend/src/ExactSplitEditor.tsx](../frontend/src/ExactSplitEditor.tsx) | Backend exact/conversion/share/report tests and frontend exact/payload/preview tests passed | Live create/edit MongoDB round-trip was not exercised |
...
| Recipient-confirmed UPI settlement tracking | Implemented and locally verified | The linked payer reviews a selected recipient, current UPI revision, trip amount, and exact INR amount; the server persists an `initiated` attempt before copy/launch. The payer reports paid or cancels; a linked creditor account or reviewer confirms, disputes, retries, or closes. Only confirmation posts one capped ledger payment; unresolved attempts expire after 24 hours without affecting balances. Attempts survive navigation/restart, block duplicate directions, show capped discrepancies, and route privacy-scoped notifications back to Settle Up. Successful confirmation uses the rich payment wording; pending/disputed workflow alerts remain generic | UPI sheet and Settle Up activity; `POST/GET /trips/{id}/payment-attempts`; sender/recipient PATCH endpoints | [backend/routes/payment_attempts.py](../backend/routes/payment_attempts.py), [backend/services/payment_attempts.py](../backend/services/payment_attempts.py), [backend/services/ledger_transactions.py](../backend/services/ledger_transactions.py), [frontend/src/UpiPaymentSheet.tsx](../frontend/src/UpiPaymentSheet.tsx), [frontend/app/trip/[id]/settle-up.tsx](../frontend/app/trip/[id]/settle-up.tsx) | Focused backend persistence/RBAC/state/expiry/cap/concurrency/void/privacy/rich-notification tests and frontend handoff/activity/routing tests pass; TypeScript passes | Successful-confirmation lock-screen copy may expose payer, receiver, posted trip amount, and trip name; UPI ID, reference, and external INR evidence remain excluded. Recipient confirmation requires a transaction-capable MongoDB deployment and fails closed otherwise. No bank verification, payment-intent callback, QR, screenshot processing, or direct money movement is performed |
...
| Dashboard, trip list, personal balances, spend ranking, and drill-downs | Implemented and verified | Home groups the authenticated person's net position by currency and shows up to the first two activity-ordered trips; Trips preserves the complete server order; trip Summary shows category/entity charts; category drill-down reconciles net/gross/refunds and ranks positive amounts by payer above amount-ordered transactions | Home/Trips, Trip Summary, and Category detail; `GET /trips`, `/balances`, `/expenses`, `/spend-summary` | [frontend/app/(tabs)/dashboard.tsx](<../frontend/app/(tabs)/dashboard.tsx#L1>), [frontend/app/(tabs)/trips.tsx](<../frontend/app/(tabs)/trips.tsx#L1>), [frontend/app/trip/[id]/category/[name].tsx](<../frontend/app/trip/[id]/category/[name].tsx#L1>), [frontend/src/categorySpend.ts](../frontend/src/categorySpend.ts#L1) | Activity-order render tests, category aggregation, Android touch callback, concrete-route navigation, category render, authenticated balance, and spend-ranking tests passed | Signed-APK physical-device verification remains post-publication; trip cards intentionally show no activity label; payer rankings count positive fronted amounts and show refunds separately |
...
| XLSX report | Implemented and verified | Downloads six tabs with whole-number ISO-coded money formats. Transactions retain canonical totals and immutable FX evidence, actual participant allocations identify remainder recipients, Payments sanitize recipient-confirmed UPI source labels, and Migration Adjustments expose the private reconciliation vector in its dedicated audit section | Reports tab; `GET /report.xlsx?token=...` | [frontend/app/(tabs)/reports.tsx](<../frontend/app/(tabs)/reports.tsx>), [backend/routes/reports.py](../backend/routes/reports.py) | Report builder/layout tests and a generated six-sheet workbook verified integers, grouping, wide values, allocations, FX rates, and adjustments | HTTP download was not run; JWT appears in the URL query string; UPI IDs/references and mobile numbers are intentionally omitted |
| PDF report | Implemented and locally verified | Full landscape report with whole-number ISO-coded money, actual participant allocations and remainder recipients, sanitized recipient-confirmed UPI payment labels, immutable FX evidence, and a dedicated migration-adjustment section | Reports tab; `GET /report.pdf?token=...` | [frontend/src/api.ts](../frontend/src/api.ts), [backend/services/report_pdf.py](../backend/services/report_pdf.py) | Generated four-page PDF text and page bounds verified complete integer values, FX metadata, sanitized UPI sources, allocations, and migration adjustments | Live browser download was not run; mobile numbers are intentionally omitted; query-string JWT has the same exposure caveat as XLSX |
...
| Health/config/categories metadata | Implemented and verified | Public health check, category list, runtime email/chat/multi-currency flags, and the application-wide `whole_unit_v1` policy/rounding contract | `GET /health`, `/meta/categories`, `/meta/config` | [backend/routes/meta.py](../backend/routes/meta.py) | Direct route and AuthContext configuration tests passed; live HTTP not run | `whole_unit_settlements_enabled: true` remains only as an old-client compatibility field; health deliberately does not prove database or provider health |
...
| Leave own settled membership | No | Yes | Yes | Yes, after automatic transfer |
| Record/edit/delete a partial payment | No | Receiver only | Yes | Yes |
| Start/report/cancel a UPI attempt | No | Linked initiating payer only | No override | No override |
| Confirm/dispute/close a UPI attempt | No | Linked creditor only | Yes | Yes |
| Mark an old pending settlement paid | No | Creditor/lender only | Yes | Yes |

Backend guards are authoritative ([backend/utils/deps.py](../backend/utils/deps.py#L20)); the frontend predicates only hide controls for better UX ([frontend/src/permissions.ts](../frontend/src/permissions.ts#L1)). Self-service departure additionally rechecks exact settlement, family rows, active UPI attempts, identity uniqueness, and ownership inside one required transaction. Owner departure transfers deterministically; no-successor trips must be deleted separately first.
...
| [05-isolate-math.md](../.claude/specs/05-isolate-math.md#L1) | Historical pure greedy settlement calculator | Superseded by the conserving scaled-integer engine; compatibility entry point retained |
...
- Email password recovery is discoverable from login whenever the backend email feature is enabled.
- Budget enforcement is complete on create but absent on edit.

### Requirements not found in current code

The older PRD still lists foreign-exchange conversion and push notifications as future work, but
both now have implementation and focused coverage. General offline synchronization remains
...
- **Older clients require an update.** PIN request fields and recovery endpoints were removed immediately; old installed builds cannot authenticate against this backend.
...
- **Live FX integration is unverified.** Ordinary tests mock Frankfurter and MongoDB. A narrowly
  scoped live provider smoke test is intentionally excluded from deterministic CI, and cache-index
  concurrency still needs staging verification.
- **Whole-unit money is application-wide.** Active budgets, expenses, converted monetary results,
  allocations, balances, settlements, payments, and UPI amounts use one-major-unit increments for
  every supported currency. Exchange-rate ratios and immutable historical FX/terminal UPI evidence
  retain their original decimal precision in audit data. Existing trips require the guarded,
  transaction-backed `migrate_whole_unit_money.py` command before they carry the policy marker.
