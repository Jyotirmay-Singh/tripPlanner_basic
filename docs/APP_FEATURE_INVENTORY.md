...
| Session persistence, logout, and route guard | Implemented and verified | Token/session is restored from AsyncStorage; logout resets navigation; signed-out users are redirected. Setup precedence is required Google password, optional mobile, optional UPI, then invitation/dashboard, with the invitation preserved throughout | Root layout, Profile, avatar header | [frontend/app/_layout.tsx](../frontend/app/_layout.tsx), [frontend/src/AuthContext.tsx](../frontend/src/AuthContext.tsx) | Logout/auth-navigation, password/mobile/UPI onboarding, and invite-handoff Jest tests passed; TypeScript passed | No refresh-token flow; an invalid/expired token clears the local session |
