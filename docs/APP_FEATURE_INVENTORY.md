...
This is a code-first inventory of the repository as it existed on 2026-08-31. Plans, roadmap checkboxes, specs, user guides, and Claude instructions were treated as supporting evidence only. A feature is described as current only when a corresponding code path was found.
...
|---|---|---|---|---|---|---|
| Revocable Android invite links | Implemented; rollout gated | Every trip member can rotate one private seven-day link and manage their own history; admins can audit/revoke all links. Installed Android apps open the token-aware join wizard, while an absent-app Android browser auto-starts the stable APK once and keeps manual/web fallbacks visible | `/invite/[token]`, trip Members tab; invite management/status APIs; token-aware join APIs | [backend/services/invites.py](../backend/services/invites.py), [frontend/app/invite/[token].tsx](../frontend/app/invite/[token].tsx), [frontend/src/InviteLinksPanel.tsx](../frontend/src/InviteLinksPanel.tsx) | Invite security/API, member/admin authorization, rotation/concurrency, landing auto-download, runtime-refresh, sharing, navigation, auth-return, and join tests plus TypeScript | `INVITE_LINKS_ENABLED` stays false until the coordinated source deploy and verified App-Link-capable APK are at `/download/android`; legacy code sharing remains the explicit fallback |
