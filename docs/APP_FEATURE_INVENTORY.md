...
| Google sign-in and first-time credential setup | Implemented but unverified | The Google button appears only when the current platform has a client ID; new OAuth users are routed to set a PIN and password | `GoogleSignInButton`, `/set-credentials`; `POST /auth/google`, `POST /auth/set-credentials` | [frontend/src/GoogleSignInButton.tsx](../frontend/src/GoogleSignInButton.tsx#L29), [backend/routes/auth.py](../backend/routes/auth.py#L184) | TypeScript passed; external OAuth was not exercised | Hidden when the platform client ID is absent; backend audiences must match the issuing platform |
...
| Dashboard, personal trip balances, spend ranking, and drill-downs | Implemented and verified | Home groups the authenticated person's net position by currency and shows recent trips; each Trips card shows that person's owed/owing/settled state; trip Summary shows budget/spend, category donut, and entity spending bars | Home/Trips and Trip Summary; `GET /balances`, `GET /spend-summary` | [frontend/app/(tabs)/dashboard.tsx](<../frontend/app/(tabs)/dashboard.tsx#L1>), [frontend/app/(tabs)/trips.tsx](<../frontend/app/(tabs)/trips.tsx#L1>), [frontend/src/tripBalance.ts](../frontend/src/tripBalance.ts#L1) | Authenticated individual/family resolution, currency aggregation, adaptive card, mixed-list, and dashboard rendering tests passed | Live API loading and Android device visual QA were not exercised; spend ranking counts positive fronted amounts and excludes refunds |
