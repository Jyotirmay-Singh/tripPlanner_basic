...
type Props = {
  ...
  subtitle?: string;
  brandIcon?: IconName;
  /** Wide brand wordmark. When provided it replaces the square medallion (e.g. the sign-in
   *  screens show the "TRIP SPLITTER" wordmark). Transparent PNG ⇒ theme-agnostic. */
  brandImage?: ImageSourcePropType;
  children: React.ReactNode;
  /** Visible navigator headers already own the top inset. */
...
export default function AuthShell({
  title, subtitle, brandIcon = 'wallet', brandImage, children, nativeHeader = false,
}: Props) {
...
const styles = StyleSheet.create({
  ...
  // Wide wordmark occupies the medallion slot; contain keeps its aspect, left-aligned like the medallion.
  wordmark: { width: 320, height: 44, maxWidth: '100%', alignSelf: 'flex-start' },
});
