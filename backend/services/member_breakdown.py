...
Two paths, chosen per family:
  ...
  * **>=1 expense restricts participation** -> each expense's owed share (PER_CAPITA *and*
    PER_FAMILY) is split only among the participating members (excluded -> 0) via
    ``allocate_within_family``; paid credits and settlements stay uniform by family size (the payer is
    the family entity, not a named member); a deterministic largest-remainder pass makes the rounded
    member shares sum EXACTLY to the family's rounded ledger net.

In both split modes only the family's INTERNAL division changes — the family's already-computed
entity total (``split_per_capita`` / ``split_per_family``), the trip headcount, the ledger ``net``,
and every other entity are untouched.
"""
...
def family_member_breakdown(members: list, expenses: list, settlements: list, net: dict) -> dict:
    ...
    for fam in members:
        ...
        owed = {mid: 0.0 for mid in ids}  # owed shares (both modes), redistributed by participation
        uniform = 0.0                      # paid credits + settlements (attributed by family size)
        restricted = False
        ...
        for e in expenses:
            ...
            if in_split:
                # The family's already-computed entity share for this expense — exactly what the
                # ledger uses. PER_CAPITA divides by total humans; PER_FAMILY is flat per entity.
                if mode == "PER_CAPITA":
